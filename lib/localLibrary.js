...
async function upsertLocalBookBatch(records) {
  return upsertLocalBookBatchWithOptions(records);
}

async function upsertLocalBookBatchWithOptions(records, { rebuildFts = true } = {}) {
  if (records.length === 0) {
    return;
  }
...
async function flushBatch(batch, counters, onProgress) {
  ...
  await upsertLocalBookBatchWithOptions(records, { rebuildFts: false });
...
export async function initializeLocalLibrary() {
  ...
  await db.execAsync(`
    ...
    CREATE INDEX IF NOT EXISTS idx_local_books_updated_at ON local_books (updated_at DESC);
    CREATE INDEX IF NOT EXISTS idx_local_books_progress ON local_books (progress DESC);
    CREATE INDEX IF NOT EXISTS idx_local_books_genre ON local_books (genre);
...
export async function getLibraryShelfBooks({ favoriteIds = [], limit = 12 } = {}) {
  ...
  const recentRows = await db.getAllAsync(
    `SELECT lb.*
     FROM reader_state rs
     JOIN local_books lb ON rs.book_id = ('local:' || lb.id)
     ORDER BY rs.updated_at DESC, rs.page_index DESC
     LIMIT ?`,
    limit
  );
  ...
  return {
    recent: fallbackRecentRows.map(mapLocalRowToBook),
