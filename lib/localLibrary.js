...
function mapLocalRowToBook(row) {
  return {
    ...
    title: row.title,
    author: row.author,
    description: row.description || "Disponible en tu dispositivo para lectura local.",
...
export async function searchLocalBooks({
  ...
  let fromClause = "FROM local_books lb";
  let orderClause = "ORDER BY lb.normalized_title ASC";
  ...
  if (hasQuery) {
    ...
    fromClause = `
      FROM local_books lb
      JOIN local_books_fts fts ON fts.rowid = lb.id
    `;
  ...
  if (hasGenres) {
    whereClauses.push(`(${genres.map(() => "lb.genre LIKE ?").join(" OR ")})`);
